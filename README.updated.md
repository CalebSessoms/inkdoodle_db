...
1. Add full `projects/*`, `chapters/*`, `notes/*`, `refs/*` IPC CRUD handlers with consistent auth checks.
2. Implement a local change queue and a safe, idempotent sync protocol (sync API + server-side merging).
3. Add field-level conflict UI and improved verification telemetry from uploads.
